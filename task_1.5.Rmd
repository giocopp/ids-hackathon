...
plot1_true_completed_survey  <- ggplot(selected_rows, aes(x = factor(used_yougov), fill = factor(used_yougov))) +
  ...
  scale_fill_manual(values = c("blue", "lightgreen"), name = "Used YouGov", labels = c("No", "Yes")) +
  labs(title = "Survey data providers", x = "Used YouGov Domain") +
  theme_minimal() +
  theme(
    text = element_text(size = 10),  # Set the default text size
    plot.title = element_text(size = 12),  # Set the title text size
    axis.title.x = element_text(size = 8)  # Set the x-axis label text size
  ) +
...
plot2 <- ggplot(yougov, aes(x = factor(group3), fill = factor(group3))) +
  geom_bar(stat = "count", show.legend = TRUE) +
  scale_fill_manual(values = c("darkred", "forestgreen"), name = "Group 3", labels = c("No", "Yes")) +
  labs(title = "All Participants", x = "Supplied web data and none for fourth survey") +
  theme_minimal() +
  theme(
    text = element_text(size = 10),  # Set the default text size
    plot.title = element_text(size = 12),  # Set the title text size
    axis.title.x = element_text(size = 7)  # Set the x-axis label text size
  ) +
  scale_y_continuous(limits = c(0, y_axis_limit))
...
plot3 <- ggplot(yougov, aes(x = factor(group4), fill = factor(group4))) +
  geom_bar(stat = "count", show.legend = TRUE) +
  scale_fill_manual(values = c("darkred", "forestgreen"), name = "Group 4", labels = c("No", "Yes")) +
  labs(title = "All Participants", x = "Supplied data for fourth survey and no tracking data.") +
  theme_minimal() +
  theme(
    text = element_text(size = 10),  # Set the default text size
    plot.title = element_text(size = 12),  # Set the title text size
